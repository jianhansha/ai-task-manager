...
class ShowTasksAgent:
    ...
    def handle_request(self, intent: str) -> list:
        # This can be upgraded later to NLP parsing or more complex filters
    ...
    def _get_today_tasks(self):
        ...
        filtered_tasks = [t for t in all_tasks if t.due_date and t.due_date.date() == today]
        return self.task_manager.tasks_to_dataframe(filtered_tasks)

    def _get_by_priority(self, priority_level):
        all_tasks = self.task_manager.get_all_tasks()
        filtered_tasks = [t for t in all_tasks if t.priority.lower() == priority_level.lower()]
        return self.task_manager.tasks_to_dataframe(filtered_tasks)